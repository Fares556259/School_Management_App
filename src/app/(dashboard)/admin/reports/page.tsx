  ...
  return (
    <div className="p-6 bg-white rounded-2xl flex-1 m-4 shadow-sm">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">{t.dailyReports.title}</h1>
          <p className="text-gray-500 text-sm mt-1">{t.dailyReports.subtitle}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="col-span-1 border rounded-xl p-5 shadow-sm bg-gray-50/50 h-fit">
          <h2 className="text-lg font-bold mb-4">{t.dailyReports.addStakeholder}</h2>
          <form action={addSubscriber} className="flex flex-col gap-4">
            <div>
              <label htmlFor="name" className="text-xs text-gray-600 font-semibold mb-1 block">{t.dailyReports.fullName}</label>
              <input
                type="text"
                id="name"
                name="name"
            ...
            <div>
              <label htmlFor="email" className="text-xs text-gray-600 font-semibold mb-1 block">{t.dailyReports.email}</label>
              <input
                type="email"
                id="email"
                name="email"
        ...
        <div className="col-span-2 border rounded-xl overflow-hidden shadow-sm">
          <div className="bg-gray-50 border-b px-5 py-3">
            <h2 className="text-lg font-bold">{t.dailyReports.activeRecipients} ({subscribers.length})</h2>
          </div>
          
          <div className="p-0">
            {subscribers.length === 0 ? (
              <div className="p-8 text-center text-gray-500">
                <p>{t.dailyReports.noSubscribers}</p>
                <p className="text-sm mt-1">{t.dailyReports.noSubscribersDesc}</p>
              </div>
            ) : (
