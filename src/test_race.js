...
async function simulateServerAction(data) {
  ...
  console.log(`Action for id: ${data.id}. Found ${existingGroupSlots.length} existing group slots. nextGroupId: ${nextGroupId}`);
...
async function main() {
  // Delete any existing slotNumber 2 for this test
  ...
  for (const sess of sessions) {
    try {
      const created = await simulateServerAction({
        id: sess.id,
        classId: 116,
        day: 'MONDAY',
        slotNumber: 2,
